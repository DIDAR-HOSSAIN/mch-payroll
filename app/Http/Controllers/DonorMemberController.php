...
use App\Http\Requests\StoreDonorMemberRequest;
use App\Http\Requests\UpdateDonorMemberRequest;
...
class DonorMemberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return inertia::render('Bm-Ex-Students-Donor-Member/ViewBmExStudentsDonorMember', ['members' => $members]);
    }
    ...
    public function create()
    {
        ...
        return inertia::render('Bm-Ex-Students-Donor-Member/CreateBmExStudentsDonorMember', ['districts' => $districts,  'message' => session('message'),]);
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(StoreDonorMemberRequest $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'nid_no' => 'nullable|string|max:17|unique:donor_members,nid_no',
                'email' => 'nullable|email|max:55|unique:donor_members,email',
                ...
                'transaction_no' => 'required|string|max:17|unique:donor_members,transaction_no',
            ...
            $data = $request->all();
            $data['donor_id'] = $this->generateExStudentsDonorId();
            ...
            $imagePath = public_path('images/donors/');
    ...
    private function generateExStudentsDonorId()
    {
        $prefix = 'DID';
        ...
        try {
            ...
            $latestMember = DB::table('donor_members')
                ->where('donor_id', 'like', "$prefix-$currentDate-%")
                ->lockForUpdate()
                ->orderBy('donor_id', 'desc')
                ->first();

            // Extract serial number
            $serialNumber = $latestMember ? intval(substr($latestMember->donor_id, -3)) + 1 : 1;
     ...
     */
    public function show(DonorMember $donor)
    {
        return inertia::render('Bm-Ex-Students-Donor-Member/ShowBmExStudentsDonorMember', [
            'member' => $donor,
        ]);
    }
     ...
     */
    public function edit(DonorMember $donor)
    {
        $districts = District::with('thanas')->get();

        return Inertia::render('Bm-Ex-Students-Donor-Member/EditBmExStudentsDonorMember', [
            'member' => $donor,
            'districts' => $districts,
     ...
     */
    public function update(UpdateDonorMemberRequest $request, DonorMember $donor)
    {
        try {
            ...
            $member->amount = $validatedData['amount'] ?? $member->amount;
     ...
     */
    public function destroy(DonorMember $donor)
    {
        if ($donor->image) {
            $imagePath = public_path('images/donors/' . $donor->image);
